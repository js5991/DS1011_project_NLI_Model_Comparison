...
if not os.path.isfile(dictpath):
    ...
    sentences_to_padded_index_sequences(
        word_indices, [training_snli, dev_snli, test_snli])
...
loaded_embeddings = loadEmbedding_rand(
    FIXED_PARAMETERS["embedding_data_path"], word_indices)
...
class modelClassifier:
    ...
    def get_minibatch(self, dataset, start_index, end_index):
        ...
        premise_vectors = np.vstack(
            [dataset[i]['sentence1_binary_parse_index_sequence'] for i in indices])
        hypothesis_vectors = np.vstack(
            [dataset[i]['sentence2_binary_parse_index_sequence'] for i in indices])
    ...
    def restore(self, best=True):
        if True:
            path = os.path.join(
                FIXED_PARAMETERS["ckpt_path"], modname) + ".ckpt_best"
        else:
            path = os.path.join(
                FIXED_PARAMETERS["ckpt_path"], modname) + ".ckpt"
        self.sess = tf.Session()
...
if test == False:
    ...
    logger.Log("Acc on SNLI test-set: %s" % (evaluate_classifier(
        classifier.classify, test_snli, FIXED_PARAMETERS["batch_size"]))[0])
else:
    ...
    results = evaluate_final(
        classifier.restore, classifier.classify, [test_snli], FIXED_PARAMETERS["batch_size"])
    ...
    logger.Log("Acc on SNLI test set: %s" % (results[0]))

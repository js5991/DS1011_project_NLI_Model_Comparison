...
epoch_number = 2000
batch_size = 32
note = "decomposable_hs_200_epoch2000_snli_char_avgemb"
...
class encoder_char(nn.Module):
    ...
    def __init__(self, num_embeddings, embedding_size, hidden_size, para_init, padding_index=100):
...
def train(batch_size, use_shrinkage, epoch_number, initial_accumulator_value, learning_rate, model_saving_dir, note, to_lower, hidden_size, glove_path, use_ngram=False, load_and_train=None):
    print("loading glove")
    ...
    if use_ngram:
        vocab_ngram, word_embeddings_ngram, ngram_to_index, index_to_ngram_map = load_embedding_and_build_vocab_ngram(glove_path)
        training_set = add_char_ngrams(training_set, build_ngram_map=False, ngram_to_index_map=ngram_to_index)
        print('training set ngrams added')
        dev_set = add_char_ngrams(dev_set, build_ngram_map=False, ngram_to_index_map=ngram_to_index)
        print('dev set ngrams added')

    train_iter = batch_iter(dataset=training_set, batch_size=batch_size, use_ngram=use_ngram, shuffle=True)
    dev_iter = batch_iter(dataset=dev_set, batch_size=batch_size, use_ngram=use_ngram, shuffle=False)
    ...
    if use_ngram:
        word_embeddings_ngram[100, :] = np.ones(300)
        word_embeddings_ngram = (word_embeddings_ngram.T / np.linalg.norm(word_embeddings_ngram, ord=2, axis=1)).T
        word_embeddings_ngram[100, :] = np.zeros(300)

        input_encoder = encoder_char(len(ngram_to_index.keys()), embedding_size=300, hidden_size=hidden_size, para_init=0.01, padding_index=1)
        input_encoder.embedding.weight.data.copy_(torch.from_numpy(word_embeddings_ngram))
    ...
    for epoch in range(epoch_number):
        ...
        valid_accuracy, valid_loss, predictions = evaluate(model, input_encoder, dev_iter, num_batch, use_cuda)
        ...
        if valid_accuracy > best_acc:
            ...
            pickle.dump(predictions, open(model_saving_dir + 'predictions' + '_' + note + '.pk', 'wb'))
...
def evaluate(inter_atten, input_encoder, data_iter, num_batch, use_cuda):
    ...
    return correct / float(total), loss_data / float(total), predictions
...
if __name__ == '__main__':
    ...
    if __name__ == '__main__':
        train(batch_size=batch_size, use_shrinkage=False, epoch_number=epoch_number, initial_accumulator_value=accu_value,
              learning_rate=learning_rate, model_saving_dir=model_saving_dir, note=note, to_lower=True, hidden_size=hidden_size, glove_path=glove_path, use_ngram=use_ngram, load_and_train=False)

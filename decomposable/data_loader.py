    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            if line['gold_label'] != '-':
                ...
                tmp1.insert(0, '<NULL>')
                tmp2.insert(0, '<NULL>')
                example['premise_sentence'] = line['sentence1']
                example['hypothesis_sentence'] = line['sentence2']
...
def batch_iter_test(dataset, batch_size, shuffle):
    start = -1 * batch_size
    dataset_size = len(dataset)
    ...
    while True:
        ...
        for k in batch:
            ...
            premise.append(k['premise_to_tokens'])
            hypothesis.append(k['hypothesis_to_tokens'])
            premise_sentence.append(k['premise_sentence'])
            hypothesis_sentence.append(k['hypothesis_sentence'])
        max_length_prem = max([len(item) for item in premise])
        max_length_hypo = max([len(item) for item in hypothesis])
        for item in premise:
            item.extend([100] * (max_length_prem - len(item)))
        for item in hypothesis:
            item.extend([100] * (max_length_hypo - len(item)))
        yield [label, premise, hypothesis, premise_sentence, hypothesis_sentence]
